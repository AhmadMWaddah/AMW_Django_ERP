...
from audit.logic.logging import log_audit, serialize_for_audit, audit_operation
...
class TestAuditLogModel:
    ...
    def test_audit_log_indexes(self):
        ...
        type_logs = AuditLog.objects.filter(
            content_type='test',
            object_id='5'
        )
...
class TestAuditOperationDecorator:
    ...
    def test_audit_operation_with_before_after(self):
        ...
        @audit_operation(
            action_code='inventory.stock.adjust',
            action='adjust',
            get_target=lambda *args, **kwargs: args[1] if len(args) > 1 else None
        )
        ...
        assert audit_entries.count() == 1
        # The decorator should have logged the operation

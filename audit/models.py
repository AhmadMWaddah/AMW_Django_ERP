...
class AuditLog(models.Model):
    ...
    actor_email = models.EmailField(blank=True, help_text="Email backup (in case user is deleted)")
    actor_name = models.CharField(max_length=200, blank=True, help_text="Name backup (in case user is deleted)")
    ...
    action_code = models.CharField(
        max_length=100, help_text="Machine-readable action code (e.g., inventory.stock.adjust)"
    )
    action_description = models.TextField(blank=True, help_text="Human-readable description of the action")

    # -- Target Information --
    content_type = models.CharField(max_length=100, help_text="Type of object affected (e.g., inventory.Product)")
    object_id = models.CharField(max_length=100, help_text="ID of the object affected")
    object_repr = models.CharField(max_length=300, help_text="String representation of the object")

    # -- State Changes --
    before_data = models.JSONField(null=True, blank=True, help_text="State before the change (for updates/deletes)")
    after_data = models.JSONField(null=True, blank=True, help_text="State after the change (for creates/updates)")
    ...
    user_agent = models.TextField(null=True, blank=True, help_text="User agent string from request")
